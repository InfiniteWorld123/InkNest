...
type RichTextEditorProps = {
	...
	onChange: (document: JSONContent, isEmpty: boolean) => void;
	isDisabled?: boolean;
};
...
export function RichTextEditor({
	...
	const editor = useEditor({
		...
		onUpdate: ({ editor: currentEditor }) => {
			onChange(currentEditor.getJSON(), currentEditor.isEmpty);
		},
		editorProps: {
			attributes: {
	...
	if (!editor) {
		return (
			<div className="space-y-2" aria-label="Loading rich text editor">
